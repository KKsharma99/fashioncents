...
function generalfeed($gender, $utype, $category, $feedtype, $number, $offset) {
  ...
  if (!empty($category)) {
    ...
    $sqlstatement .= " DISTINCT a.postid as postid, b.userid as userid, b.description as description, b.image_small as image_small, b.posttime as posttime, b.hash as hash, c.username as username, c.utype as utype, c.profilepic as profilepic";
    if ($feedtype >= 5) {
    ...
    $sqlstatement .= " b.postid as postid, b.userid as userid, b.description as description, b.image_small as image_small, b.posttime as posttime, b.hash as hash, c.username as username, c.utype as utype, c.profilepic as profilepic";
    if ($feedtype >= 5) {
      $sqlstatement .= ", COUNT(d.userid)";
    }
    if ($feedtype == 3) {
...
function parsefeed($sqlresult) {
	...
	while ($row = mysqli_fetch_assoc($sqlresult)) {
		...
		$post = $row;
		$details = array();
		$detailsql = "SELECT id, itemname, brand, merchant, price, image, link FROM tbl_items WHERE postid = " . $post['postid'];
		$detailresult = mysqli_query($GLOBALS['sqllink'], $detailsql);
		while ($detailrow = mysqli_fetch_assoc($detailresult)) {
      $name = $detailrow['itemname']; 
      if(strlen($name) > 30) {
        $maxLength = 30; // maximum number of characters to extract
        //trim the string to the maximum length
        $name = substr($name, 0, $maxLength);
        //re-trim if we are in the middle of a word
        $name = substr($name, 0, min(strlen($name), strrpos($name, " "))) . "...";
        $detailrow['itemname'] = $name;
      }
...
function generatepost($post, $details, $following, $comments, $commentlikes, $usercommentlikes, $commentcount, $likes, $userlike, $saved) {
	?>
	<div id ="post<?php echo($post['postid']) ?>" class="col-lg-4 col-md-6"> 
    <div class="panel panel-white post panel-shadow">

      <div class="post-heading">

        <div class="pull-left image">
          <a href="account.php?usr=<?php echo($post['username']) ?>">
        ...
        <div class="pull-left">
          <div class="post-user">
            <a href="account.php?usr=<?php echo($post['username']) ?>"><b><?php echo($post['username']) ?></b></a> <br>
            <span class="text-muted post-time"><?php echo(nicetime($post['posttime'])) ?></span>
          </div>
        </div> <!-- User Name --> 

        <div class="dropdown">

          <btn name="followbtn<?php echo($post['userid']) ?>" class="dropdown-toggle" type="button" data-toggle="dropdown"><img data-toggle="tooltip" title="OPTIONS" src="vendor/custom-icons/dots-v.png" class="dots-v pull-right"></btn>
        ...
        <?php if ($_SESSION['id'] == $post['userid']) { ?>
              <img data-toggle="tooltip" title="DELETE" id="follow<?php echo($post['postid']) ?>" name="follow<?php echo($post['userid']) ?>" src="vendor/custom-icons/circle-cross.png" class="circle-plus pull-right" onclick = "showDeleteModal(<?php echo($post['postid']); ?>)">
        <?php } else { ?>
        <?php if (!$following) { ?>

        <btn id="follow<?php echo($post['postid']) ?>" name="follow<?php echo($post['userid']) ?>" class="btn btn-sm btn-primary pull-right follow-btn" onclick = "follow(<?php echo($post['userid'] . ", " . $_SESSION['id']); ?>, 1, '<?php echo($_SESSION['username']) ?>');">FOLLOW</btn>

        <?php } else { ?>

        <btn id="follow<?php echo($post['postid']) ?>" name="follow<?php echo($post['userid']) ?>" class="btn btn-sm btn-primary pull-right follow-btn" onclick = "follow(<?php echo($post['userid'] . ", " . $_SESSION['id']); ?>, 2, '<?php echo($_SESSION['username']) ?>');">FOLLOWING</btn>
      ...
      <div class="post-description bottom-padding"> 
        <?php 
        if(strpos($_SERVER['PHP_SELF'],'index.php') !== false || strpos($_SERVER['PHP_SELF'],'feedgenerator.php') !== false) {
          echo '<a href="singlepost_' . $post['hash'] . '.php">';
        } else {
          echo '<a>';
        } ?>
          <?php if($userlike) { ?>
          ...
          <?php } else { ?>
            <img id = "img<?php echo($post['postid'] ) ?>"" class="img-responsive text-center" width="100%" src="<?php echo($post['image_small']) ?>" alt="test" ondblclick = "like(<?php echo($post['postid'] . ", 1, " . $_SESSION['id'] .",". $post['userid'] . ",'" .  $_SESSION['username'] . "'") ?>)">
          <?php } ?>
        ...
        <div class="new-interact-bar"> 
          ...
          <?php if($userlike) {?>
              <img  id = "like<?php echo($post['postid']) ?>" data-toggle="tooltip" title="LIKE" src="vendor/custom-icons/heart2.png" class="emoj-reacts" onclick="like(<?php echo($post['postid'] . ", 2, " . $_SESSION['id'] .",". $post['userid'] . ",'" . $_SESSION['username']."'") ?>)">
          <?php } else { ?>
              <img  id = "like<?php echo($post['postid']) ?>" data-toggle="tooltip" title="LIKE" src="vendor/custom-icons/heart.png" class="emoj-reacts" onclick="like(<?php echo($post['postid'] . ", 1, " . $_SESSION['id'] .",". $post['userid'] . ",'" .  $_SESSION['username'] . "'") ?>)">
          <?php } ?>
          
           <a data-toggle="modal" data-target="#likesmodal<?php print($post['postid']); ?>" title="See who liked this post.">
      ...
      <?php include 'likemodal.php'; ?>

      <div class="post-footer new-post-footer">
        <ul id = "commentlist<?php echo($post['postid']) ?>">
        <?php if (!empty($post['description'])) { ?>
          <li>
          <div class="post-likes-box"></div>
            <p class="post-comment-text">
              <a href="account.php?usr=<?php echo($post['username']) ?>"><strong><?php echo($post['username']) ?></strong>
              </a> <?php echo($post['description']) ?>
            </p>
        ...
        ?>
          ...
          <div class="post-likes-box">
              <!--<a href="account.php?usr=<?php echo($comments[$com]['username']) ?>">-->
            ...
            <p class="post-comment-text">
              <a href="account.php?usr=<?php echo($comments[$com]['username']) ?>"><?php echo($comments[$com]['username']) ?>
              </a> <?php echo($comments[$com]['commenttext']) ?>
            </p>

          <div class="post-time-report-box">

               <span class="pull-right comment-date"><?php echo(nicetime($comments[$com]['fctime'])) ?></span>       
         ...
         <!-- This Script Initializes the Tooltips --> 
          ...
          <!-- End Tooltip Initializing Script -->
              ...
              <?php for($item = 0; $item < count($details) && $item < 4; $item++) { 
                  generateItemElement($details[$item]);
              } 
              if (count($details) > 4) { ?>


            <span class="collapse" id="moreItems<?php echo($post['postid']) ?>"> <!-- Start More Items Hidden Section -->
           	  <?php for($item = 4; $item < count($details); $item++) { 
                  generateItemElement($details[$item]);
              } ?>
            </span> <!-- End More Items Hidden Section -->
            <div class="text-center">
              <img aria-controls="addComment" aria-expanded="false" data-target="#moreItems<?php echo($post['postid']) ?>" data-toggle="collapse"
  ...
  <div id ="post<?php echo($post['postid']) ?>" class="col-lg-4"> 
    <div class="panel panel-white post panel-shadow">
      ...
      <div class="post-heading">
        ...
        <div class="pull-left">
          <div class="post-user">
            <a data-toggle="modal" data-target="#guestprompt"><b><?php echo($post['username']) ?></b></a> <br>
            <span class="text-muted post-time"><?php echo(nicetime($post['posttime'])) ?></span>
          </div>
        </div> <!-- User Name --> 
        ...
        <a data-toggle="modal" data-target="#guestprompt">
         <btn id="follow<?php echo($post['postid']) ?>" name="follow<?php echo($post['userid']) ?>" class="btn btn-sm btn-primary pull-right follow-btn");">FOLLOW</btn>
        </a>
      </div> <!-- End Post Heading --> 

      <div class="post-description bottom-padding"> 
        <?php if(strpos($_SERVER['PHP_SELF'],'index.php') !== false || strpos($_SERVER['PHP_SELF'],'feedgenerator.php') !== false) {
          echo '<a href="singlepost_' . $post['hash'] . '.php">';
        } else {
          echo '<a>';
        } ?>
        ...
        <div class="dflpost-footer">
         ...
         <!-- This Script Initializes the Tooltips --> 
          ...
          <!-- End Tooltip Initializing Script -->
            ...
            <div class="text-center">
              <img aria-controls="addComment" aria-expanded="false" data-target="#moreItems<?php echo($post['postid']) ?>" data-toggle="collapse"
              data-toggle="tooltip" title="COMMENT" src="vendor/custom-icons/down-arrow.png" class="more-items-dots">
            </div> 
...
<?php } 
function generateItemElement($detail) { ?>
              <li class="product-item">
                <a class="pull-left" target="_blank" href="<?php echo($detail['link']) ?>" onclick = "clicktrack(<?php echo($_SESSION['visitid'] . ", " . $detail['id'] . ", 1"); ?>);">
                  <img class="img-rounded avatar" src="<?php if (empty($detail['image'])) echo("img/icon.png"); else echo($detail['image']); ?>" alt="item">
                </a>
                <a class="pull-right" target="_blank" href="<?php echo($detail['link']) ?>" onclick = "clicktrack(<?php echo($_SESSION['visitid'] . ", " . $detail['id'] . ", 2"); ?>);">
                  <button type="button" class="btn btn-sm btn-success buy-button"><?php if (empty($detail['price'])) { echo("BUY"); } else { echo("$" . $detail['price']); } ?></button>
                </a>
                <div class="product-item-body"> <!-- Item Name, Merchant, Brand, Price --> 
                  <div class="product-item-details">
                    <p class="item-name"><?php echo($detail['itemname']) ?></p>
                    <p class="item-brand"><?php echo($detail['brand']) ?></p>
                    <p class="item-merchant"><?php echo($detail['merchant']) ?></p>
                  </div>
                </div>
              </li>
<?php } ?>